...
function __promptline_battery {
  ...
  for possible_battery_dir in /sys/class/power_supply/BAT*; do
    if [[ -d $possible_battery_dir && -f "$possible_battery_dir/energy_full" && -f "$possible_battery_dir/energy_now" ]]; then
      ...
      printf "%s" "${sad_smiley}${battery_symbol}${battery_level}${percent_sign}"
      return
    fi
  done
...
function __promptline_ps1 {
  ...
  # section "a" slices
  __promptline_wrapper "$(printf "%s" \\u; )" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
